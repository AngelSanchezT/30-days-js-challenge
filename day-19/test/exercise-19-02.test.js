import { processPay } from "../exercise-19-02";
...
describe("🌐 Day 19-02 - Payment system", () => {
  ...
  it("Card makePay method should throw an error", () => {
    ...
    expect(() => rta.makePay(122)).to.throw();
  });
  ...
  it("processPay should call to makePay method", () => {
    ...
    expect(processPay(cash, 200)).to.eql({
      realized: true,
      quantity: 200,
    });
    expect(processPay(card, 100)).to.eql({
      realized: true,
      quantity: 100,
      lastCardNumbers: "1122",
    });
    expect(processPay(paypal, 300)).to.eql({
      realized: true,
      quantity: 300,
      platform: "PayPal",
      email: "test@mail.com",
